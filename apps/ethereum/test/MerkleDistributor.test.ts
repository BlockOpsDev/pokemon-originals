...
const gasUsed = {
    MerkleDistributor: {
        ...
        largerTreeFirstClaim: 85319,
        largerTreeSecondClaim: 68219,
        realisticTreeGas: 95324,
        realisticTreeGasDeeperNode: 95296,
        realisticTreeGasAverageRandom: 78706,
        realisticTreeGasAverageFirst25: 62440,
    },
    MerkleDistributorWithDeadline: {
        twoAccountTree: 82100,
        largerTreeFirstClaim: 85451,
        largerTreeSecondClaim: 68351,
        realisticTreeGas: 95456,
        realisticTreeGasDeeperNode: 95428,
        realisticTreeGasAverageRandom: 78838,
        realisticTreeGasAverageFirst25: 62572,
    },
}
...
for (const contract of ['MerkleDistributor', 'MerkleDistributorWithDeadline']) {
    describe(`${contract} tests`, () => {
        ...
        beforeEach(async () => {
            ...
            wallet0 = wallets[0]
            wallet1 = wallets[1]
            const tokenFactory = await ethers.getContractFactory('MockERC20', wallet0)
        ...
        describe('#claim', () => {
            ...
            describe('two account tree', () => {
                ...
                it('cannot allow two claims', async () => {
                    const proof0 = tree.getProof(0, wallet0.address, BigNumber.from(100))
                    await distributor.claim(0, wallet0.address, 100, proof0, overrides)
                    await expect(distributor.claim(0, wallet0.address, 100, proof0, overrides)).to.be.revertedWithCustomError(distributor, 'AlreadyClaimed')
                })
                ...
                it('cannot claim for address other than proof', async () => {
                    const proof0 = tree.getProof(0, wallet0.address, BigNumber.from(100))
                    await expect(distributor.claim(1, wallet1.address, 101, proof0, overrides)).to.be.revertedWithCustomError(distributor, 'InvalidProof')
                })

                it('cannot claim more than proof', async () => {
                    const proof0 = tree.getProof(0, wallet0.address, BigNumber.from(100))
                    await expect(distributor.claim(0, wallet0.address, 101, proof0, overrides)).to.be.revertedWithCustomError(distributor, 'InvalidProof')
                })
            ...
            describe('realistic size tree', () => {
                ...
                it('no double claims in random distribution', async () => {
                    for (let i = 0; i < 25; i += Math.floor(Math.random() * (NUM_LEAVES / NUM_SAMPLES))) {
                        ...
                        await expect(distributor.claim(i, wallet0.address, 100, proof, overrides)).to.be.revertedWithCustomError(distributor, 'AlreadyClaimed')
                    }
                })
            })
...
describe('#MerkleDistributorWithDeadline', () => {
    ...
    it('cannot claim after end time', async () => {
        ...
        const proof0 = tree.getProof(0, wallet0.address, BigNumber.from(100))
        await expect(distributor.claim(0, wallet0.address, 100, proof0, overrides)).to.be.revertedWithCustomError(distributor, 'ClaimWindowFinished')
    })
